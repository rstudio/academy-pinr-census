...
plot_over_time <- function(measure, df = acs, by = sex, facet = region) {
  ...
  if (!any(str_detect(colnames(measure), "male")) ||
      !any(str_detect(colnames(measure), " to ")) ||
      !all(numeric_measures)) {
    cli::cli_abort("`measure` must include numeric variables partitioned by sex and age.")
  }
  ...
  if (is.numeric(facet_data)) {
    cli::cli_abort("The `facet` column must be {.cls character} or {.cls factor}, not {.cls {class(facet_data)}}.")
  }
